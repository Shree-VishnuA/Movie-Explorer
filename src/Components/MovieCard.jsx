...
function MovieCard({ movie = {} }) {
  ...
  <>
    ...
    <div
      className="w-full max-w-xs mx-auto bg-black/40 backdrop-blur-md border border-white/30 rounded-xl shadow-lg hover:shadow-[0_0_20px_#f67c02] transition-all duration-300 hover:scale-[1.03] cursor-pointer overflow-hidden group"
    ...
    {isModalOpen && (
      ...
      >
        <div
          className="bg-black/70 backdrop-blur-md border border-white/10 rounded-xl p-6 max-w-lg w-full text-gray-100 relative shadow-2xl overflow-y-auto max-h-[90vh]"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Close Button */}
          <div
            onClick={() => setIsModalOpen(false)}
            className="absolute top-3 right-3 text-gray-300 text-2xl rounded-full hover:text-red-400 cursor-pointer"
          >
            <X />
          </div>
          ...
          <div className="flex flex-col gap-2 text-sm text-gray-400">
            ...
            {movie?.original_language && (
              <span>Language: {getLanguageName(movie.original_language)}</span>
            )}
